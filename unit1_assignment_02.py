...
def get_encoding_dict():
    ...
    d2=dict(zip(string.ascii_uppercase,range(1,27)))
    d1.update(d2)
    return d1
...
def get_consonents(input):
    ...
    No control flow plz.
    """
    return  set(input.lower()).difference(VOWEL_SET)
...
def bit_count_even(number):
    ...
    """
    c=bin(number).count('1', 0, len(bin(number)))
    return (c+1)%2
